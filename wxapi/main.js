...
const API_BASE_URL = 'https://api.24ud.cn'


const request = (url, needSubDomain, method, data, formData, header) => {
  ...
  return new Promise((resolve, reject) => {
    wx.request({
      ...
      success(request) {
        ...
        resolve( header ? request : request.data)
      },
      fail(error) {
        reject(error)
      },
...
module.exports = {
  ...
  // app登录
  login: (data) => {
    return request('/courier/couriers/login', true, 'post', data)
  },
  // 获取签收列表
  getSignList(data) {
    return request('/order/orders', true, 'get', data, false, true)
  }
}
